...
Uses the real PostgreSQL database via Docker.
 
It uses the following pattern:
  - Module-level engine + NullPool (no connection pooling in tests)
  - get_db dependency override so the app uses the test session
  - _client() is a plain helper function, NOT a pytest fixture
  - _payload() helpers build request bodies with safe defaults
  - autouse cleanup fixture removes all test_ prefixed users after every test
  - Every async test is decorated with @pytest.mark.asyncio
  - _create_user() seeds users directly into the DB for login/refresh/logout tests
    (because those tests need an active user without going through /register)
 
All test users use the 'test_' prefix so the autouse cleanup fixture
...
def _client() -> AsyncClient:
    """Returns a fresh AsyncClient for each test"""
...
async def _create_user(
    ...
    """
    Inserts a user directly into the DB, bypassing /register.
    Used by login/refresh/logout tests that need a pre-existing user
    without going through registration flow.
    ON CONFLICT DO NOTHING means it is safe to call multiple times.
    """
...
def cleanup_test_users():
    ...
    Deletes all users whose username starts with 'test_'
    and their associated refresh tokens.
    Uses asyncio.run() to execute the async delete from a sync fixture,
    matching the exact pattern from the register tests in this project.
    """
    yield
