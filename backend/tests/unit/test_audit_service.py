...
async def test_export_csv_includes_header_and_resolved_usernames(
    ...
    mock_user_repo = AsyncMock()
    mock_user_repo.get_username_by_id.return_value = "jane_admin"
...
async def test_export_csv_empty_result_returns_header_only(fake_audit_log):
    mock_repo = AsyncMock()
    mock_repo.list_all_logs.return_value = []
    service = AuditService(mock_repo)
