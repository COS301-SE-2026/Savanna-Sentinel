...
export function MotionSimulator() {
    ...
    const [preset, setPreset] = useState<"manual" | "walking" | "shaking">("manual");
            ...
            {isEnabled && (
                <div>
                    <div className="grid grid-cols-3 gap-1">
                        {(["manual", "walking", "shaking"] as const).map((mode) => (
                            <button
                                key={mode}
                            ...
                            {(["x", "y", "z"] as const).map((axis) => (
                                <div key={axis} className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        value={[axis]}
                                        onChange={(e) =>
                                            setMotion({ ...motion, [axis]: Number.parseFloat(e.target.value) })
                                        }
                                    />
                                    <span>
                                        {motion[axis].toFixed(1)}
                                    </span>
                                </div>
                            ))}
                            <div>
                            ...
                            {(["alpha", "beta", "gamma"] as const).map((axis) => (
                                <div key={axis} className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        value={rotation[axis]}
                                        onChange={(e) =>
                                            setRotation({ ...rotation, [axis]: Number.parseFloat(e.target.value) })
                                        }
                                    />
                                    <span>
                                        {rotation[axis]}
