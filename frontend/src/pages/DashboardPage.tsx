...
function severityBadge(severity: string | null) {
    ...
    if (severity === "High")
        return (
            <Badge className="bg-spot-red/10 text-spot-red border-spot-red/20">
                {severity}
            </Badge>
        );
    if (severity === "Medium")
        return (
            <Badge className="bg-spot-orange/10 text-spot-orange border-spot-orange/20">
                {severity}
            </Badge>
        );
    return (
        <Badge className="bg-spot-green/10 text-spot-green border-spot-green/20">
            {severity}
        </Badge>
    );
}
                        ...
                        {RISK_ZONES.map((z) => (
                            <div key={z.zone}>
                                <div className="flex justify-between text-sm mb-1">
                                    ...
                                    <span
                                        className={`text-xs font-semibold ${z.level === "Critical" ? "text-spot-red" : z.level === "High" ? "text-spot-orange" : z.level === "Medium" ? "text-amber-500" : "text-spot-green"}`}
                                    >
                                        {z.level}
                                    </span>
                                </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <p className="text-xs text-muted-foreground pt-1">
                            Metrics are computed on the last validated dataset
                            upload. Do not interpret these as guarantees of
                            field accuracy.
                        </p>
                    </CardContent>
                </Card>
