...
async function fillValidForm(overrides: Record<string, string> = {}) {
  ...
  await userEvent.type(screen.getByLabelText(/first name/i), values.first_name);
...
describe("RegisterPage", () => {
  ...
  it("shows the success screen after a valid submission", async () => {
    ...
    expect(screen.getByText(/pending admin activation/i)).toBeInTheDocument();
  ...
  it("shows a 409 error when username or email is already taken", async () => {
    ...
    expect(banner).toHaveTextContent(/email or username is already in use/i);
  });

  it("renders all three role options in the dropdown", async () => {
    renderRegisterPage();
    await userEvent.click(screen.getByRole("combobox"));
    expect(await screen.findByRole("option", { name: /^ranger$/i })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: /^analyst$/i })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: /^community liaison$/i })).toBeInTheDocument();
  });
