...
describe("RoleSwap - Role Management", () => {
  ...
  it("shows an error message when fetching users fails", async () => {
    server.use(
      http.get("**/v1/users", () => new HttpResponse(null, { status: 500 }))
    );
  ...
  it("save button enables after selecting a different role", async () => {
    ...
    const analystOption = await screen.findByRole("option", { name: /analyst/i });
  ...
  it("shows success message after a successful role change", async () => {
    ...
    expect(await screen.findByText(/role updated to analyst/i)).toBeInTheDocument();
  });

  it("shows error message when role change fails", async () => {
    server.use(
      http.patch("**/v1/users/:id/role", () =>
  ...
  it("success message disappears after 5 seconds", async () => {
    ...
    await waitFor(
      () => expect(screen.queryByText(/role updated to analyst/i)).not.toBeInTheDocument(),
      { timeout: 6000 }
    );
  }, 10000);
});
